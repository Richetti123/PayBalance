...
let handler = async (m, { conn, text, command, usedPrefix }) => {
    ...
    if (!clientIdentifier) {
        return m.reply(`*Uso incorrecto:*\nPor favor, proporciona el nombre o número de WhatsApp del cliente a activar.\nEjemplo: \`\`\`${usedPrefix}${command} Juan\`\`\` o \`\`\`${usedPrefix}${command} 5217771234567\`\`\``);
    }
    ...
    try {
        ...
        clientsData[clientFoundKey].suspendido = false; // Marcar como NO suspendido

        fs.writeFileSync(paymentsFilePath, JSON.stringify(clientsData, null, 2), 'utf8');
        await m.reply(`✅ Cliente *${clientsData[clientFoundKey].nombre || clientFoundKey}* ha sido *activado*.\nAhora recibirá recordatorios e interacciones del bot.`);

    } catch (e) {
        console.error('Error processing .activarcliente command:', e);
        m.reply(`❌ Ocurrió un error interno al intentar activar al cliente. Por favor, reporta este error.`);
    }
};

handler.help = ['activarcliente <nombre/número>'];
handler.tags = ['pagos'];
handler.command = /^(activarcliente)$/i;
handler.owner = true;
