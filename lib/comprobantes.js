...
export async function handleIncomingMedia(m, conn) {
    ...
    if (m.message.buttonsResponseMessage && m.key.remoteJid === ADMIN_NUMBER_FOR_FORWARDING) {
        ...
        // --- LOGS ADICIONALES PARA DEPURACIÓN EN COMPROBANTES.JS ---
        console.log('\n--- DEBUG: Admin Button Response Handler in comprobantes.js ---');
        console.log('selectedButtonId:', selectedButtonId);
        console.log('originalMessage EXISTS:', !!originalMessage);
        if (originalMessage) {
            console.log('Keys of originalMessage:', Object.keys(originalMessage));
            console.log('originalMessage.imageMessage EXISTS:', !!originalMessage.imageMessage);
            if (originalMessage.imageMessage) console.log('originalMessage.imageMessage keys:', Object.keys(originalMessage.imageMessage));
            console.log('originalMessage.documentMessage EXISTS:', !!originalMessage.documentMessage);
            if (originalMessage.documentMessage) console.log('originalMessage.documentMessage keys:', Object.keys(originalMessage.documentMessage));
            console.log('originalMessage.videoMessage EXISTS:', !!originalMessage.videoMessage);
            if (originalMessage.videoMessage) console.log('originalMessage.videoMessage keys:', Object.keys(originalMessage.videoMessage));
            console.log('originalMessage.extendedTextMessage EXISTS:', !!originalMessage.extendedTextMessage);
            if (originalMessage.extendedTextMessage) {
                console.log('originalMessage.extendedTextMessage keys:', Object.keys(originalMessage.extendedTextMessage));
                if (originalMessage.extendedTextMessage.contextInfo) {
                    console.log('originalMessage.extendedTextMessage.contextInfo keys:', Object.keys(originalMessage.extendedTextMessage.contextInfo));
                    console.log('originalMessage.extendedTextMessage.contextInfo.quotedMessage EXISTS:', !!originalMessage.extendedTextMessage.contextInfo.quotedMessage);
                }
            }
        }
        console.log('--- END DEBUG: Admin Button Response Handler in comprobantes.js ---\n');
        ...
        if (originalMessage.imageMessage) {
            ...
            isImageForSave = false; 
            console.warn('DEBUG: Se detectó un video como comprobante.');
        } 
        // Si no se encuentra directamente, buscar en un quotedMessage anidado (común con extendedTextMessage)
            ...
            if (nestedQuotedMessage.imageMessage) {
                ...
                isImageForSave = true;
                console.log('DEBUG: Media encontrada anidada en originalMessage.extendedTextMessage.contextInfo.quotedMessage (imagen).');
            } else if (nestedQuotedMessage.documentMessage) {
                messageContentForSave = nestedQuotedMessage.documentMessage;
                isImageForSave = false;
                console.log('DEBUG: Media encontrada anidada en originalMessage.extendedTextMessage.contextInfo.quotedMessage (documento).');
            } else if (nestedQuotedMessage.videoMessage) {
                messageContentForSave = nestedQuotedMessage.videoMessage;
                isImageForSave = false;
                console.log('DEBUG: Media encontrada anidada en originalMessage.extendedTextMessage.contextInfo.quotedMessage (video).');
            }
        }

        if (!messageContentForSave) {
            console.error('ERROR: No se pudo encontrar contenido multimedia válido en el comprobante original.');
        ...
        if (selectedButtonId.startsWith('accept_payment_')) {
            ...
            if (paymentsData[clientNumber]) {
                try {
                    // ¡IMPORTANTE! LLAMADA DIRECTA A processPaymentProofAndSave
                    console.log('DEBUG: Llamando a processPaymentProofAndSave desde comprobantes.js');
                    console.log('DEBUG: messageContentForSave keys (antes de llamar):', Object.keys(messageContentForSave));
                    console.log('DEBUG: isImageForSave (antes de llamar):', isImageForSave);
