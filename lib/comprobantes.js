...
// Asegúrate de que esta ruta sea correcta para tu archivo subircomprobante.js
...
export async function handleIncomingMedia(m, conn) {
    ...
    if (m.message.buttonsResponseMessage && m.key.remoteJid === ADMIN_NUMBER_FOR_FORWARDING) {
        ...
        // --- Inicio de la lógica crucial para extraer el contenido del comprobante ---
        let messageContentForSave;
        let isImageForSave = false;

        // Verificar si el originalMessage tiene una imagen o un documento
        if (originalMessage.imageMessage) {
            ...
            // Si el mensaje original no es una imagen ni un documento, no podemos procesarlo
            console.error('Mensaje original no contiene ni imagen ni documento para guardar.');
            await conn.sendMessage(m.chat, { text: '❌ Error: El comprobante original no es una imagen ni un documento válido.' });
            return true;
        }
        ...
        if (selectedButtonId.startsWith('accept_payment_')) {
            ...
            if (paymentsData[clientNumber]) {
                ...
                try {
                    // LLAMADA DIRECTA A processPaymentProofAndSave (¡Este es el cambio clave!)
